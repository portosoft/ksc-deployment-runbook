...
## 2025-02-28 - [CRITICAL] Prevent TOCTOU credential exposure in SFTP file creation
**Vulnerability:** A Python script (`automation/setup/reconfigure_ksc_service.py`) was writing a file containing passwords (`/tmp/reconfig_ans.txt`) via Paramiko's `sftp.file("...", "w")` without setting permissions first. This creates a Time-of-Check to Time-of-Use (TOCTOU) vulnerability where the file is created with default permissions (often world-readable in `/tmp`) before the sensitive content is written.
**Learning:** When using Paramiko SFTP to write sensitive data to temporary or shared directories, the file must have its permissions explicitly restricted immediately upon creation and before any data is written to it. Relying on default umasks or applying `sftp.chmod()` after closing the file leaves a window of opportunity for local attackers to read the file contents.
**Prevention:** Always call `f.chmod(0o600)` on the file object immediately after opening it with `sftp.file("...", "w")` and *before* calling `f.write(...)`. This ensures the file is secured against unauthorized access before the sensitive payload is written.
