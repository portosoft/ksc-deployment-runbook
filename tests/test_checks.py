...
from automation.python.checks import check_os_version, check_ram_and_disk, check_selinux, check_ports
...
def test_check_ram_and_disk_critical(mock_disk, mock_ram, dummy_config):
    result = check_ram_and_disk(dummy_config)
    assert result.has_critical
    assert len([i for i in result.items if i.status == "critical"]) == 3 # ram, opt, varopt
