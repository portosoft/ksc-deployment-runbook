...
def debug_core_init(host, user, password):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command(f"sudo -S cp /tmp/server-base.js {base_file}")
        stdin.write(password + '\n')
        stdin.flush()

        # 2. Inject into server/core/env-local/web-server.js
        print("--- Injecting into server/core/env-local/web-server.js ---")
        local_file = "/var/opt/kaspersky/ksc-web-console/server/core/env-local/web-server.js"
        ...
        client.exec_command(f"base64 -d /tmp/web-server-local.txt > /tmp/web-server-local.js")
        stdin, stdout, stderr = client.exec_command(f"sudo -S cp /tmp/web-server-local.js {local_file}")
        stdin.write(password + '\n')
        stdin.flush()
