...
def update_to_fqdn_and_restart(host, user, password):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command("sudo -S cp /tmp/config_fqdn.json /var/opt/kaspersky/ksc-web-console/server/config.json")
        stdin.write(password + '\n')
        stdin.flush()
        
        # 4. Deep Restart
        print("--- Deep Restarting Services ---")
        client.exec_command("sudo -S systemctl stop KSCWebConsole.service KSCSvcWebConsole.service")
        client.exec_command("sudo -S killall -9 node")
        import time
        time.sleep(2)
        client.exec_command("sudo -S systemctl start KSCSvcWebConsole.service KSCWebConsole.service")
        stdin.write(password + '\n')
        stdin.flush()
