...
def phase_1_audit_refined(host, user, password):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command('rpm -V ksc-web-console | grep -E "^[SM5]{1,8}"')
        print(stdout.read().decode('utf-8'))

        # 1.3 Config JSON
        print("--- 1.3 Config JSON content ---")
        stdin, stdout, stderr = client.exec_command('sudo -S find /etc/kaspersky /var/opt/kaspersky -name "config.json"')
