...
def check_web_pid_ports(host, user, password):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command("ps aux | grep 'node ./index.js' | grep -v grep | awk '{print $2}'")
        pid = stdout.read().decode('utf-8').strip()
        print(f"PID: {pid}")

        if pid:
            print(f"--- Checking ports for PID {pid} ---")
            stdin, stdout, stderr = client.exec_command(f"sudo -S ss -tulpn | grep {pid}")
