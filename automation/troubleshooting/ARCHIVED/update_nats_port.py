...
def update_ports_and_deploy(host, user, password):
    ...
    try:
        ...
        client.exec_command(f'echo "{password}" | sudo -S mv /tmp/start_console.js {remote_js} && echo "{password}" | sudo -S chown root:root {remote_js}')
        ...
        with sftp.file('/tmp/pm.config.js', 'w') as f:
            f.write(new_pm)
        client.exec_command(f'echo "{password}" | sudo -S mv /tmp/pm.config.js {target_pm} && echo "{password}" | sudo -S chown root:root {target_pm}')
