...
"""
        ...
        with sftp.file('/tmp/nats.conf', 'w') as f:
            f.write(nats_conf)
        sftp.close()
        client.exec_command(f'echo "{password}" | sudo -S mv /tmp/nats.conf {target_dir}/nats.conf')
        ...
        with sftp.file('/tmp/pm.config.js', 'w') as f:
            f.write(new_pm)
        sftp.close()
        client.exec_command(f'echo "{password}" | sudo -S mv /tmp/pm.config.js {target_dir}/pm.config.js')
        ...
        client.exec_command(f'echo "{password}" | sudo -S systemctl start ksc-nats.service')
        client.exec_command(f'echo "{password}" | sudo -S systemctl start ksc-web-console.service')
