...
def robust_fix_web_server(host, user, password):
    ...
    try:
        ...
        content = re.sub(
            r"runtime\.logger\.info\(DEBUG: openAPIServers",
            "runtime.logger.info('DEBUG: openAPIServers'",
            content,
        )
        content = re.sub(
            r"runtime\.logger\.info\(DEBUG: serversCount",
            "runtime.logger.info('DEBUG: serversCount'",
            content,
        )
        ...
        client.exec_command(
            f"base64 -d /tmp/web-server-b64.txt > /tmp/web-server-fixed.js"
        )
