...
def find_servers_count_loc(host, user, password):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command(
            "sudo -S grep -n 'serversCount' /var/opt/kaspersky/ksc-web-console/server/core/web-server.js"
        )
        ...
        stdin, stdout, stderr = client.exec_command(
            "sudo -S grep -n 'serversCount' /var/opt/kaspersky/ksc-web-console/server/core/env-local/web-server.js"
        )
        stdin.write(password + "\n")
        stdin.flush()
        print(stdout.read().decode("utf-8"))
