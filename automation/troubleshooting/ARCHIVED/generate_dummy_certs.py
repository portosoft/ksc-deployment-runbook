...
def generate_dummy_certs(host, user, password):
    ...
    try:
        ...
        # Generate Root CA
        cmd = (
            f"sudo openssl genrsa -out {target_dir}/KLRootCA.key 2048 && "
        ...
        cmd = (
            f"sudo openssl genrsa -out {target_dir}/web-server.key 2048 && "
        ...
        # Permissions
        client.exec_command(
            f"sudo chown root:root {target_dir}/web-server.* {target_dir}/KLRootCA.*"
        )
        client.exec_command(
            f"sudo chmod 644 {target_dir}/web-server.* {target_dir}/KLRootCA.*"
        )
