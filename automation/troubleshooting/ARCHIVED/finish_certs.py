...
def finish_certs(host, user, password):
    ...
    try:
        ...
        client.exec_command(f'echo "{password}" | sudo -S rm {target_dir}/web-server.csr {target_dir}/KLRootCA.srl')
        client.exec_command(f'echo "{password}" | sudo -S chown root:root {target_dir}/web-server.* {target_dir}/KLRootCA.*')
        client.exec_command(f'echo "{password}" | sudo -S chmod 644 {target_dir}/web-server.* {target_dir}/KLRootCA.*')
