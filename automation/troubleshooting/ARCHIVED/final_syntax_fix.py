...
class WebServer extends web_server_1.WebServer {
    constructor() {
        super();
        this.serversCount = 0;
        runtime.logger.error('DEBUG: openAPIServers', JSON.stringify(global.config.openAPIServers));
        runtime.logger.error('DEBUG: serversCount', this.serversCount);
...
"""
        ...
        client.exec_command(f"base64 -d /tmp/web-server-final.txt > /tmp/web-server-fixed.js")
...
if __name__ == "__main__":
    ...
    final_syntax_fix(host, user, password)
