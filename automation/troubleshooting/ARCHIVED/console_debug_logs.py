...
def console_debug_logs(host, user, password):
    ...
    try:
        ...
        if "async handleLoginRequest(req, res) {" in content:
            content = content.replace("async handleLoginRequest(req, res) {", "async handleLoginRequest(req, res) {\n        console.error('DEBUG: handleLoginRequest serversCount', this.serversCount);")
        ...
        client.exec_command(f"base64 -d /tmp/web-server-console.txt > /tmp/web-server-fixed.js")
