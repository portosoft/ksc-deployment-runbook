...
def fix_web_console_with_certs(host, user, password):
    ...
    try:
        ...
        client.exec_command(
            'openssl req -x509 -newkey rsa:2048 -keyout /tmp/key.pem -out /tmp/cert.pem -days 365 -nodes -subj "/CN=kscserver"'
        )
        client.exec_command(
            "sudo -S cp /tmp/key.pem /tmp/cert.pem /var/opt/kaspersky/ksc-web-console/"
        )
        stdin, _, _ = client.exec_command(
            "sudo -S chown ksc:kladmins /var/opt/kaspersky/ksc-web-console/*.pem"
        )
