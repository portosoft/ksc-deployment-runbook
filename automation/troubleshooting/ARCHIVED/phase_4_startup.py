...
def phase_4_startup(host, user, password):
    ...
    try:
        ...
        stdin, stdout, stderr = client.exec_command('sudo -S systemctl start KSCWebConsole.service')
        ...
        stdin, stdout, stderr = client.exec_command('sudo -S journalctl -u KSCWebConsole.service --no-pager -n 100')
        stdin.write(password + '\n')
        stdin.flush()
        print(stdout.read().decode('utf-8'))
        ...
        stdin, stdout, stderr = client.exec_command('curl -k https://127.0.0.1:8080 -o /dev/null -w "%{http_code}" 2>/dev/null')
