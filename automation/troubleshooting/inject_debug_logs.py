...
def inject_debug_logs(host, user, password):
    ...
    try:
        ...
        if "this.serversCount = 0;" in content:
            new_content = content.replace("this.serversCount = 0;", "this.serversCount = 0;" + debug_log)
            
            # Write back
            client.exec_command(f"echo '{new_content}' > /tmp/web-server-debug.js")
            stdin, stdout, stderr = client.exec_command(f"sudo -S cp /tmp/web-server-debug.js {target_file}")
            stdin.write(password + '\n')
            stdin.flush()
