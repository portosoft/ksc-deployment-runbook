...
def reconfigure_ksc_service(config: KscConfig, apply: bool = False) -> None:
    ...
    # Unique filename to prevent predictable temporary file vulnerabilities (CWE-377)
    ans_filename = f"/tmp/reconfig_ans_{uuid.uuid4().hex}.txt"
...
"""
    ...
    if not apply:
        run_logger.info(
            f"[CHECK] Seria gerado um arquivo de respostas KLAUTOANSWERS em '{ans_filename}' via SFTP (modo 0600)."
        )
        run_logger.info(
            f"[CHECK] Seria executado: KLAUTOANSWERS={ans_filename} /opt/kaspersky/ksc64/lib/bin/setup/postinstall.pl"
        )
        run_logger.info(
            f"[CHECK] O arquivo temporário '{ans_filename}' seria removido do servidor remoto."
        )
        run_logger.info(
    ...
    try:
        ...
        run_logger.info(
            f"Gerando arquivo de respostas em {ans_filename} via SFTP..."
        )
        sftp = client.open_sftp()
        f = sftp.file(ans_filename, "w")
