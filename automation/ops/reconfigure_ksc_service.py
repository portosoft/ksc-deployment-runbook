...
"""
    ...
    if not apply:
        run_logger.info(
            "[CHECK] Seria gerado um arquivo de respostas KLAUTOANSWERS em '/tmp/reconfig_ans_*.txt' via SFTP (modo 0600)."
        )
        run_logger.info(
            "[CHECK] Seria executado: KLAUTOANSWERS=/tmp/reconfig_ans_*.txt /opt/kaspersky/ksc64/lib/bin/setup/postinstall.pl"
        )
        run_logger.info(
            "[CHECK] O arquivo temporário '/tmp/reconfig_ans_*.txt' seria removido do servidor remoto."
        )
        run_logger.info(
    ...
    try:
        ...
        run_logger.info(
            "Gerando arquivo de respostas em /tmp/reconfig_ans_*.txt via SFTP..."
        )
