...
def reconfigure_ksc_service(config: KscConfig, apply: bool = False) -> None:
    ...
    temp_file_path = f"/tmp/reconfig_ans_{temp_uuid}.txt"
...
"""
    ...
    if not apply:
        run_logger.info(
            f"[CHECK] Seria gerado um arquivo de respostas KLAUTOANSWERS em '{temp_file_path}' via SFTP (modo 0600)."
        )
        run_logger.info(
            f"[CHECK] Seria executado: KLAUTOANSWERS={temp_file_path} /opt/kaspersky/ksc64/lib/bin/setup/postinstall.pl"
        )
        run_logger.info(
            f"[CHECK] O arquivo temporário '{temp_file_path}' seria removido do servidor remoto."
        )
        run_logger.info(
    ...
    try:
        ...
        run_logger.info(
            f"Gerando arquivo de respostas em {temp_file_path} via SFTP..."
        )
        sftp = client.open_sftp()
        f = sftp.file(temp_file_path, "w")
        ...
        postinstall_cmd = (
            f"KLAUTOANSWERS={temp_file_path} "
        ...
        client.exec_command(f"rm -f {temp_file_path}")
