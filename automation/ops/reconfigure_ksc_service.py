...
"""
    ...
    tmp_ans_file = f"/tmp/reconfig_ans_{uuid.uuid4().hex}.txt"

    if not apply:
        run_logger.info(
            f"[CHECK] Seria gerado um arquivo de respostas KLAUTOANSWERS em '{tmp_ans_file}' via SFTP (modo 0600)."
        )
        run_logger.info(
            f"[CHECK] Seria executado: KLAUTOANSWERS={tmp_ans_file} /opt/kaspersky/ksc64/lib/bin/setup/postinstall.pl"
        )
        run_logger.info(
            f"[CHECK] O arquivo temporário '{tmp_ans_file}' seria removido do servidor remoto."
        )
        run_logger.info(
    ...
    try:
        ...
        run_logger.info(
            f"Gerando arquivo de respostas em {tmp_ans_file} via SFTP..."
        )
        sftp = client.open_sftp()
        f = sftp.file(tmp_ans_file, "w")
        ...
        client.exec_command(f"rm -f {tmp_ans_file}")
