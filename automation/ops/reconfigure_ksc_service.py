...
"""
    ...
    # Gera um nome de arquivo aleatório para evitar colisão e previsibilidade
    tmp_filename = f"/tmp/reconfig_ans_{uuid.uuid4().hex}.txt"

    if not apply:
        run_logger.info(
            f"[CHECK] Seria gerado um arquivo de respostas KLAUTOANSWERS em '{tmp_filename}' via SFTP (modo 0600)."
        )
        run_logger.info(
            f"[CHECK] Seria executado: KLAUTOANSWERS={tmp_filename} /opt/kaspersky/ksc64/lib/bin/setup/postinstall.pl"
        )
        run_logger.info(
            f"[CHECK] O arquivo temporário '{tmp_filename}' seria removido do servidor remoto."
        )
        run_logger.info(
    ...
    try:
        ...
        run_logger.info(
            f"Gerando arquivo de respostas em {tmp_filename} via SFTP..."
        )
        sftp = client.open_sftp()
        f = sftp.file(tmp_filename, "w")
        ...
        client.exec_command(f"rm -f {tmp_filename}")
