...
"""
    ans_path = f"/tmp/reconfig_ans_{uuid.uuid4().hex}.txt"

    if not apply:
        run_logger.info(
            f"[CHECK] Seria gerado um arquivo de respostas KLAUTOANSWERS em '{ans_path}' via SFTP (modo 0600)."
        )
        run_logger.info(
            f"[CHECK] Seria executado: KLAUTOANSWERS={ans_path} /opt/kaspersky/ksc64/lib/bin/setup/postinstall.pl"
        )
        run_logger.info(
            f"[CHECK] O arquivo temporário '{ans_path}' seria removido do servidor remoto."
        )
        run_logger.info(
    ...
    try:
        ...
        run_logger.info(
            f"Gerando arquivo de respostas em {ans_path} via SFTP..."
        )
        sftp = client.open_sftp()
        f = sftp.file(ans_path, "w")
        ...
        client.exec_command(f"rm -f {ans_path}")
