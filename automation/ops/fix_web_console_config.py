...
def fix_web_console_config(config: KscConfig, apply: bool = False) -> None:
    ...
    expr1 = r"s/\$web_console_port\$/8080/g"
    expr2 = rf"s/\$web_console_address\$/{safe_fqdn}/g"
    expr3 = r's/"port": "13000"/"port": "13299"/g'

    sed_cmd = (
        "sed -i "
        f"-e {shlex.quote(expr1)} "
        f"-e {shlex.quote(expr2)} "
        f"-e {shlex.quote(expr3)} "
