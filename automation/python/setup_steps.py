...
def _set_selinux_mode(mode: str, logger: logging.Logger) -> None:
    ...
    try:
        ...
        logger.warning(f"Aviso: Falha ao tentar mudar SELinux para {mode}. Talvez não instalado? Erro: {e}")
...
def install_ksc_server(config: KscConfig, logger: logging.Logger) -> None:
    ...
    logger.info("Nota sobre LD_LIBRARY_PATH: O KSC no Linux muitas vezes exige bibliotecas. A recomendação DEVSECOPS é usar Environment=LD_LIBRARY_PATH=... no arquivo de serviço systemd, nunca em /etc/profile ou /etc/environment, para evitar vazamento global.")
...
def perform_setup(config: KscConfig, logger: logging.Logger) -> None:
    ...
    try:
        ...
        if result.has_critical:
            raise SetupError("Prechecks falharam criticamente. Interrompendo instalação.")
            
        # Temporariamente permissivo para a instalação não falhar no setup de binários
        if original_selinux == "enforcing":
            logger.info("Desabilitando SELinux (permissive) temporariamente para a instalação do KSC.")
        ...
        if selinux_changed:
            logger.info("Restaurando SELinux para o modo enforcing (Segurança por Padrão).")
