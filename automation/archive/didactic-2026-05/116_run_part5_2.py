...
try:
    ...
    out, err = run_sudo("sudo -u postgres pg_dump ksc > /tmp/ksc_backup_$(date +%Y%m%d_%H%M).sql && ls -lh /tmp/ksc_backup_*.sql")
    print("5.1 Backup:", out, err)
    
    # 5.2 Parar todos os servicos KSC
    out, err = run_sudo("systemctl stop KSCWebConsole KSCSvcWebConsole kliam_srv kladminserver_srv")
...
"""
    ...
    out, err = run_sudo("KSC_ADMIN_USER=KLAdmins KSC_ADMIN_PASS='Ksc@2026' python3 /tmp/fix_2fa_flow.py")
