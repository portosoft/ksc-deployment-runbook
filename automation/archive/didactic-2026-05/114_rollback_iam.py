...
run_sudo("sudo -S -u postgres psql -c \"SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = 'ksciam' AND pid <> pg_backend_pid();\"")
run_sudo("sudo -S -u postgres psql -c \"DROP DATABASE IF EXISTS ksciam;\"")
run_sudo("sudo -S -u postgres psql -c \"CREATE DATABASE ksciam OWNER kluser ENCODING 'UTF8' LC_COLLATE 'pt_BR.UTF-8' LC_CTYPE 'pt_BR.UTF-8';\"")
