...
services_status = send_and_wait(
    "systemctl status kladminserver_srv klwebsrv_srv kliam_srv klnagent_srv --no-pager"
)

# Print safely in Windows CLI
try:
    sys.stdout.write(
        services_status.encode(sys.stdout.encoding, errors="replace").decode(
            sys.stdout.encoding
        )
    )
except Exception:
