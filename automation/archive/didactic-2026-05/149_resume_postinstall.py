...
# We increase the number of iterations to allow full paging.
for i in range(400):
    time.sleep(0.15)
    if shell.recv_ready():
        chunk = shell.recv(4096).decode("utf-8", errors="replace")
    ...
    if (
        "kaspersky security network" in lower_log
        and "ksn" in lower_log
        and re.search(r"\[[nr]\]:\s*$", log)
    ):
        shell.send("Y\n")
        log = ""
    ...
    if "administration server ip address or fqdn" in lower_log and re.search(
        r"\]:\s*$", log
    ):
    ...
    if (
        "enter the password of ksc administrator account:" in lower_log
        and log.endswith(": ")
    ):
        shell.send("Ksc@2026\n")
        log = ""
    ...
    if (
        "completed successfully" in lower_log
        or "concluída com êxito" in lower_log
        or "concluída com sucesso" in lower_log
        or "[root@kscserver ~]#" in log
    ):
